...
const Typography: React.ForwardRefRenderFunction<
  HTMLElement,
  TypographyProps
> = ({ className, level, weight, as: Component = "span", ...props }, ref) => {
  ...
  return (
    <Component
      className={cn(typographyVariants({ level, weight, className }))}
      ref={handleRef}
      {...props}
    />
  );
